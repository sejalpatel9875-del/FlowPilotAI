...
class LearningAgent(BaseAgent):
    def __init__(self):
        super().__init__(
            AgentMetadata(
                name="LearningAgent",
                description="Skill gap assessment, spaced repetition planning, and learning roadmap generation.",
                purpose="Evaluate technical/business skill gaps, build structured learning roadmaps, and recommend study concepts.",
                system_policy=(
                    "You are LearningAgent, FlowPilot AI's professional growth and skill acquisition agent. "
                    "Synthesize learning goals, skills, and knowledge vault docs into actionable learning plans."
                ),
                allowed_tools=["READ_SKILLS", "READ_GOALS", "READ_KNOWLEDGE", "CREATE_LEARNING_RECOMMENDATIONS"],
                allowed_data_scopes=["skills", "goals", "learning_plans", "knowledge"],
                risk_level="LOW"
            )
        )

    async def get_context(self, user_id: str, prompt: str, db: Any) -> Dict[str, Any]:
        context_str = await AgentContextBuilder.build_learning_context(user_id, prompt, db)
    ...
    async def run(self, user_id: str, prompt: str, db: Any, request_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        full_prompt = f"Skills & Learning Context:\n{ctx['context_text']}\n\nUser Instruction:\n{sanitized_prompt}"
        req = LLMRequest(
            prompt=full_prompt,
