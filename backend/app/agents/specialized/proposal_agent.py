...
class ProposalAgent(BaseAgent):
    def __init__(self):
        super().__init__(
            AgentMetadata(
                name="ProposalAgent",
                description="Freelance proposal creation, scope of work definition, and pricing breakdown.",
                purpose="Draft comprehensive client proposals with project milestones, deliverables, and terms.",
                system_policy=(
                    "You are ProposalAgent, FlowPilot AI's proposal and scope definition agent. "
                    "Synthesize client requirements into structured, high-value proposal drafts. "
                    "Do NOT sign, accept, or finalize contracts autonomously."
                ),
                allowed_tools=["READ_PROPOSALS", "READ_CLIENTS", "CREATE_PROPOSAL_DRAFT"],
                allowed_data_scopes=["proposals", "clients", "leads", "knowledge"],
                risk_level="MEDIUM"
            )
        )

    async def get_context(self, user_id: str, prompt: str, db: Any) -> Dict[str, Any]:
        context_str = await AgentContextBuilder.build_proposal_context(user_id, prompt, db)
    ...
    async def run(self, user_id: str, prompt: str, db: Any, request_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        full_prompt = f"Proposal Context:\n{ctx['context_text']}\n\nUser Instruction:\n{sanitized_prompt}"
        req = LLMRequest(
            prompt=full_prompt,
