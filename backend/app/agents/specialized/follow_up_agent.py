...
class FollowUpAgent(BaseAgent):
    def __init__(self):
        super().__init__(
            AgentMetadata(
                name="FollowUpAgent",
                description="Automated multi-touch follow-up planning and sequence drafting.",
                purpose="Monitor unreplied leads, design follow-up cadence, and draft personalized follow-up sequences.",
                system_policy=(
                    "You are FollowUpAgent, FlowPilot AI's follow-up strategy and sequence drafting agent. "
                    "Analyze conversation status and recommend appropriate follow-up timing and message drafts. "
                    "Respect automatic stop conditions when leads reply."
                ),
                allowed_tools=["READ_LEADS", "READ_CONVERSATION_STATUS", "CREATE_FOLLOWUP_DRAFT"],
                allowed_data_scopes=["leads", "follow_ups", "sequences"],
                risk_level="MEDIUM"
            )
        )

    async def get_context(self, user_id: str, prompt: str, db: Any) -> Dict[str, Any]:
        context_str = await AgentContextBuilder.build_followup_context(user_id, prompt, db)
    ...
    async def run(self, user_id: str, prompt: str, db: Any, request_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        full_prompt = f"Follow-Up Cadence Context:\n{ctx['context_text']}\n\nUser Instruction:\n{sanitized_prompt}"
        req = LLMRequest(
            prompt=full_prompt,
